...
class _AnimalReportSheetState extends State<AnimalReportSheet> {
  ...
  void initState() {
    ...
    _weightCtrl   = TextEditingController(text: a['weight_avg_kg']       != null ? '${a['weight_avg_kg']}'   : '');
    _heightCtrl   = TextEditingController(text: a['height_avg_m']        != null ? '${a['height_avg_m']}'    : '');
    _lifespanCtrl = TextEditingController(text: a['lifespan_avg_years']  != null ? '${a['lifespan_avg_years']}' : '');
    _speedCtrl    = TextEditingController(text: a['max_speed_kmh']       != null ? '${a['max_speed_kmh']}'   : '');
  }

  @override
  void dispose() {
    for (final c in [
      _nameCtrl, _emailCtrl, _nameViCtrl, _nameEnCtrl, _sciNameCtrl,
      _descCtrl, _funFactCtrl, _imageUrlCtrl, _noteCtrl,
      _weightCtrl, _heightCtrl, _lifespanCtrl, _speedCtrl,
    ]) {
      c.dispose();
    }
    super.dispose();
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      addIfChanged('lifespan_avg_years', _lifespanCtrl.text, a['lifespan_avg_years']);
      ...
      await _service.submitReport(
        ...
        reporterName:          _nameCtrl.text.trim().isNotEmpty  ? _nameCtrl.text.trim()  : null,
        reporterEmail:         _emailCtrl.text.trim().isNotEmpty ? _emailCtrl.text.trim() : null,
        suggestedNameVi:       changedOrNull(_nameViCtrl,  a['name_vietnamese']),
        suggestedNameEn:       changedOrNull(_nameEnCtrl,  a['name_english']),
        suggestedScientificName: changedOrNull(_sciNameCtrl, a['scientific_name']),
  ...
  Widget build(BuildContext context) {
    ...
    return DraggableScrollableSheet(
      ...
      builder: (_, scrollCtrl) => Container(
        ...
        child: _submitted ? _buildSuccess(colorScheme) : _buildForm(colorScheme, scrollCtrl),
      ),
    );
  }
    ...
    return Form(
      ...
      child: ListView(
        ...
        children: [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text('Báo cáo thông tin sai', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800, color: cs.onSurface)),
                    Text('Chỉnh sửa trực tiếp những gì bạn cho là không chính xác', style: TextStyle(fontSize: 12, color: cs.onSurfaceVariant)),
                  ],
                ),
              ),
            ],
          ...
          _buildTextField(_nameCtrl,  'Họ tên', Icons.person_outline_rounded, cs, required: false),
          const SizedBox(height: 10),
          _buildTextField(_emailCtrl, 'Email liên hệ', Icons.email_outlined, cs, required: false, keyboardType: TextInputType.emailAddress),

          const SizedBox(height: 24),
          _sectionHeader('Tên & Phân loại', cs),
          const SizedBox(height: 12),
          _buildTextField(_nameViCtrl,  'Tên tiếng Việt', Icons.translate_rounded, cs),
          const SizedBox(height: 10),
          _buildTextField(_nameEnCtrl,  'Tên tiếng Anh', Icons.language_rounded, cs),
          const SizedBox(height: 10),
          _buildTextField(_sciNameCtrl, 'Tên khoa học', Icons.biotech_rounded, cs),

          const SizedBox(height: 24),
          _sectionHeader('Chỉ số sinh học', cs),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: _buildTextField(_weightCtrl,   'Cân nặng (kg)', Icons.monitor_weight_outlined, cs, keyboardType: TextInputType.number)),
            const SizedBox(width: 10),
            Expanded(child: _buildTextField(_heightCtrl,   'Chiều cao (m)',  Icons.height_rounded,           cs, keyboardType: TextInputType.number)),
          ]),
          const SizedBox(height: 10),
          Row(children: [
            Expanded(child: _buildTextField(_lifespanCtrl, 'Tuổi thọ (năm)', Icons.hourglass_empty_rounded, cs, keyboardType: TextInputType.number)),
            const SizedBox(width: 10),
            Expanded(child: _buildTextField(_speedCtrl,    'Tốc độ (km/h)',  Icons.speed_rounded,            cs, keyboardType: TextInputType.number)),
          ]),

          const SizedBox(height: 24),
          _sectionHeader('Mô tả & Fun fact', cs),
          const SizedBox(height: 12),
          _buildTextField(_descCtrl,    'Mô tả ngắn', Icons.description_outlined, cs, maxLines: 4),
          const SizedBox(height: 10),
          _buildTextField(_funFactCtrl, 'Fun fact tiếng Việt', Icons.lightbulb_outline_rounded, cs, maxLines: 3),

          const SizedBox(height: 24),
          _sectionHeader('Hình ảnh', cs),
          const SizedBox(height: 12),
          _buildTextField(_imageUrlCtrl, 'URL hình ảnh mới', Icons.image_outlined, cs, keyboardType: TextInputType.url),

          const SizedBox(height: 24),
          _sectionHeader('Ghi chú thêm', cs),
          const SizedBox(height: 12),
          _buildTextField(_noteCtrl, 'Giải thích thêm hoặc nguồn tham khảo...', Icons.notes_rounded, cs, maxLines: 3, required: false),
  ...
  Widget _sectionHeader(String title, ColorScheme cs) {
    return Text(title, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: cs.onSurfaceVariant, letterSpacing: 0.5));
  }
  ...
  Widget _buildTextField(
    ...
    return TextFormField(
      ...
      validator: required
          ? (v) => (v == null || v.trim().isEmpty) ? 'Vui lòng điền thông tin' : null
          : null,
    );
  }
}