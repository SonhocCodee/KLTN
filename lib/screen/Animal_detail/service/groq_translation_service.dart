...
// Dịch fun fact và mô tả ngắn sang tiếng Anh rồi lưu lại Supabase.
// AnimalDetailScreen gọi service này sau khi load dữ liệu con vật.
class GroqTranslationService {
  // Groq
  static const String _groqUrl =
      'https://api.groq.com/openai/v1/chat/completions';
  static const String _groqKey =
      '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'; // Sau này nên đưa key qua --dart-define.
  ...
  Future<bool> translateAndSave(Map<String, dynamic> animal) async {
    ...
    final funFactVi = (animal['fun_fact_vietnamese'] as String? ?? '').trim();
    final descVi = (animal['description_short'] as String? ?? '').trim();
    final nameEn = (animal['name_english'] as String? ?? 'this animal').trim();
    final id = animal['id'];

    if (funFactVi.isEmpty && descVi.isEmpty)
      return false; // Không có gì để dịch
    ...
    _patchSupabase(
      ...
      descriptionEnglish: missingDesc ? result['description_english']! : null,
    );
...
''';
    ...
    try {
      ...
      final descEn = (parsed['description_english'] as String? ?? '').trim();

      if (funFactEn.isEmpty && descEn.isEmpty) return null;

      print('[Groq] ✅ Dịch thành công');
      return {'fun_fact_english': funFactEn, 'description_english': descEn};
    } catch (e) {
