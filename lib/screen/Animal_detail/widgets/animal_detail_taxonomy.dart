...
class AnimalDetailTaxonomy extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (animal['class'] != null) {
      items.add({'icon': '📦', 'label': t.tr('Lớp'), 'value': animal['class']});
    }
    if (animal['order_name'] != null) {
      items.add({
        'icon': '📂',
        'label': t.tr('Bộ'),
        'value': animal['order_name'],
      });
    }
    ...
    if (animal['genus'] != null) {
      items.add({'icon': '🔍', 'label': t.tr('Chi'), 'value': animal['genus']});
    }
    ...
    return Column(
      ...
      children: [
        ...
        Padding(
          ...
          child: Text(
            t.tr('Nguồn: Hệ thống phân loại sinh vật học hiện đại'),
            style: TextStyle(
              fontSize: 11,
