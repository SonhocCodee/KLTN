...
class _AnimalDetailSoundState extends State<AnimalDetailSound> {
  ...
  Future<void> _fetchSounds() async {
    ...
    try {
      ...
      if (taxonResults == null || taxonResults.isEmpty)
        throw Exception('No taxon');
      ...
      for (final o in allObs) {
        ...
        final numIdDisagree =
            (o['num_identification_disagreements'] ?? 0) as int;
      ...
      // Nếu vẫn rỗng, dự phòng không filter (lấy thẳng 3 cái đầu)
      if (top.isEmpty) {
  ...
  Widget _buildEmptyState(ColorScheme cs, LocaleProvider t) => Container(
    ...
    child: Row(
      children: [
        ...
        Text(
          t.tr('Chưa có âm thanh cho loài này'),
  ...
  Widget _buildSoundPlayer(ColorScheme cs, LocaleProvider t) {
    ...
    return Column(
      children: [
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Text(
                          '${t.tr('Ghi âm')} ${_currentIndex + 1}/${_sounds.length}',
                          style: TextStyle(
                            fontSize: 13,
        ...
        Text(
          '${t.tr('Nguồn:')} iNaturalist · ${current['attribution']}',
          style: TextStyle(
            fontSize: 11,
