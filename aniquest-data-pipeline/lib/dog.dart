...
// ═══════════════════════════════════════════════════════════════
// 🎯 ANIQUEST - UNIVERSAL ANIMAL DATA FETCHER (OPTIMIZED)
// ═══════════════════════════════════════════════════════════════
...
class AnimalConfig {
  static const String ANIMAL_TYPE = 'dog';
  static const String TRAITS_TABLE = 'dog_traits';
  static const String ANIMAL_NAME_VI = 'Chó';
  static const String ANIMAL_NAME_EN = 'Dog';

  static const List<String> BREED_LIST = [

  ];

  static const String WIKI_KEYWORD = 'dog';
}
...
void main() async {
  ...
  for (int i = 0; i < AnimalConfig.BREED_LIST.length; i++) {
    ...
    print('\n[$i/${AnimalConfig.BREED_LIST.length}] 🔍 $breed');
...
Future<Map<String, dynamic>?> _fetchWikiData(String breedName) async {
  try {
    ...
    return {
      'text': (page['extract'] ?? '').toString().substring(0, 800.clamp(0, (page['extract'] ?? '').toString().length)),
    ...
    try {
      final response = await http.post(
        ...
        body: json.encode({
          ...
          'messages': [{'role': 'user', 'content': _buildPrompt(breedName, wikiData['text'])}],
          'temperature': 0.1,
        }),
      );
...
String _buildPrompt(String breedName, String wikiText) {
  return '''Parse $breedName dog breed into JSON based on this Wikipedia info:

$wikiText

CRITICAL RULES:
1. Use 1-5 scale for integer traits (energy, trainability, etc). NEVER >5
2. temperament must be: gentle, neutral, aggressive, timid, or territorial
3. coat_length: hairless, short, medium, long, extra_long
4. grooming_needs/exercise_needs: low, moderate, high
...

{
  "animal": {
    "animal_type": "dog",
    "name_vietnamese": "Chó [Vietnamese translation of $breedName]",
    ...
    "relative_size": "medium_dog",
    "weight_avg_kg": 20.0,
    "height_avg_m": 0.5,
    "length_avg_m": 0.8,
    "primary_colors": ["black","brown","white"],
    "patterns": ["solid"],
    "fur_type": "short_fur",
    "max_speed_kmh": 40,
    "temperament": "gentle",
